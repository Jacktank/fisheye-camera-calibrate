    ...
    for(int i = 0; i < n_img; ++i)
    {
        ...
        if (found)
        {
            ...
            list_detected.push_back(list[i]);
				...
				{
						std::cout<<"fail to find all corners in image "<<list[i]<<std::endl;
				}
    }
    if (!img.empty())
        imageSize = img.size();
...
static bool readStringList( const string& filename, vector<string>& l )
{
    l.resize(0);
		...
		{
				std::cout<<"succeed to read "<<s<<std::endl;
				l.push_back(s);
		}
    return true;
}
...
int main(int argc, char** argv)
{
    ...
    saveCameraParams(outputFilename, flags, K, D, _xi, rvecs, tvecs, detec_list, idx, rms, imagePoints);
		...
		std::cout<<"xi.size():"<<xi.rows<<":"<<xi.cols<<std::endl;
		cv::Size new_size(imageSize.width,imageSize.height);//imageSize;

		cv::Matx33f Knew = Matx33f(new_size.width/7, 0, new_size.width/2,
               0, new_size.height/7, new_size.height/2,
               0, 0, 1);	

		//cv::Mat distorted = cv::imread(detec_list[0]);
		cv::Mat distorted = cv::imread("11.jpg");

		if(distorted.empty())
		{
				std::cout<<"fail to load the distored image"<<std::endl;
				return -1;
		}

		std::cout<<detec_list[0]<<std::endl;
		cv::Mat undistorted;
	  int flag = omnidir::RECTIFY_PERSPECTIVE;
		cv::omnidir::undistortImage(distorted, undistorted, K, D, xi, flag, Knew, new_size);

		cv::imshow("a",undistorted);
